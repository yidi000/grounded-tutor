...
const ERROR_COPY: Record<string, string> = {
  ...
  external_service_error: "当前无法完成这项操作；输入仍保留，可以手动重试。",
};
...
export function SourceWizard({ workspaceId, capabilities, open, initialFile, existingSource, intent, onClose, onChanged }: SourceWizardProps) {
  ...
  function validatedSettings() {
    const result = ChunkSettingsSchema.safeParse(state.settings);
    if (result.success) return result.data;
    const field = result.error.issues[0]?.path[0];
    if (field) document.getElementById(String(field))?.focus();
    dispatch({ type: "failed", error: "处理设置有误，请检查当前数值。", resumeStage: "settings" });
    return null;
  }
  ...
  async function estimate() {
    ...
    if (state.input.kind === "text" && (!textName.trim() || !text.trim())) {
      ...
      dispatch({ type: "failed", error: "请填写资料名称和内容。", resumeStage: "settings" });
      return;
    }
    try {
  ...
  return (
    <div className="dialog-backdrop" role="presentation" onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); chooseFile(event.dataTransfer.files[0]); }}>
      <section ref={dialogRef} className="dialog-card source-wizard" role="dialog" aria-modal="true" aria-labelledby="source-wizard-title">
        <div className="dialog-title-row"><div><span className="eyebrow">资料向导</span><h2 id="source-wizard-title">{title}</h2></div><button className="icon-button" type="button" onClick={onClose} aria-label="关闭资料向导">×</button></div>
        <ol className="wizard-steps" aria-label="处理步骤"><li className={stepClass(state.stage, 0)}>选择资料</li><li className={stepClass(state.stage, 1)}>处理方式</li><li className={stepClass(state.stage, 2)}>预计片段</li><li className={stepClass(state.stage, 3)}>实际复核</li></ol>
        ...
        {state.stage === "settings" && state.input && (
          <div className="wizard-body">
            ...
            <ProcessingSettings value={state.settings} capabilities={capabilities} onChange={(settings) => dispatch({ type: "settings", settings })} />

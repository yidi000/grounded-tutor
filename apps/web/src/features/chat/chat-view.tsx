...
export type ChatExchange = { question: string; response: ChatResponse; legacy_content?: string | null };
...
export function ChatView({
  ...
  return (
    <section className="chat-view" aria-label="对话">
      {exchanges.map(({ question, response, legacy_content }) => {
        ...
        return (
          <div className="chat-exchange" key={response.message_id}>
            ...
            <div className="assistant-message">
              ...
              {legacy_content ? <div><p>{legacy_content}</p><small>早期回答，未保存结构化引用。</small></div> : response.status === "insufficient_material" ? (
